...
class EncodingPriorNetwork(nn.Module):
    def __init__(
            self,
            dim,
            num_timesteps=None,
        ...
        if self.learned_query_mode == 'pos_emb':
            scale = dim ** -0.5
            self.learned_query = nn.Parameter(torch.randn(num_voxel_tokens, dim) * scale)
        if self.learned_query_mode == 'all_pos_emb':
            scale = dim ** -0.5
            self.learned_query = nn.Parameter(torch.randn(num_voxel_tokens * 2 + 1, dim) * scale)
        self.causal_transformer = FlaggedCausalTransformer(dim=dim, causal=causal, **kwargs)

        self.null_brain_embeds = nn.Parameter(torch.randn(num_tokens, dim))
        self.null_image_embed = nn.Parameter(torch.randn(num_voxel_tokens, dim))

        self.num_tokens = num_tokens
        self.num_voxels = num_voxels
        self.num_voxel_tokens = num_voxel_tokens
        self.self_cond = False

        self.voxel_encoder = nn.Sequential(
            nn.Linear(num_voxels, voxel_hidden),
            nn.Linear(voxel_hidden, num_voxel_tokens*dim),
            Rearrange('b (n d) -> b n d', n=num_voxel_tokens),
        )
        self.voxel_decoder = nn.Sequential(
            Rearrange('b n d -> b (n d)', n=num_voxel_tokens),
            nn.Linear(num_voxel_tokens*dim, voxel_hidden),
            nn.Linear(voxel_hidden, num_voxels),
        )
    ...
    def forward(
        ...
        image_embed = self.voxel_encoder(image_embed)
        ...
        # get learned query, which should predict the image embedding (per DDPM timestep)
        pred_image_embed = tokens[..., -self.num_voxel_tokens:, :]

        pred_image_embed = self.voxel_decoder(pred_image_embed)
...
class VoxelEncoder(nn.Module):
    def __init__(self, num_voxels, token_dim, num_tokens, hidden_dim, n_blocks, drop=.15):
        super().__init__()
        self.lin0 = nn.Linear(num_voxels, hidden_dim, bias=True)
        ...
        self.lin1 = nn.Linear(hidden_dim, num_tokens*token_dim)
...
class VoxelAutoEncoder(nn.Module):
    ...
    def forward(self, x):
        voxel_rep = self.voxel_encoder(x)
        pred_voxel = self.voxel_decoder(voxel_rep)
        return voxel_rep, pred_voxel